...
async function run(entries: Array<string>, command: any) {
  ...
  if (command.name() === 'watch' || command.name() === 'serve') {
    parcel.watch().subscribe({
      error(e) {
        // In watch mode, build exceptions won't be thrown beyond Parcel, so anything
        // making it here has not been through a reporter and is likely a critical
        // error. Log it and exit the watch process.
        console.error(e);
        process.exit(1);
      }
    });
    ...
    try {
      ...
      // buildFailure event. If an exception is thrown outside of Parcel.build,
      // it won't go through reporters and should be logged.
